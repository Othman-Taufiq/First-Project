...
function is_connected(){
    if(!empty($_SESSION) && !empty($_SESSION['is_login']) && $_SESSION['is_login'] == true && !empty($_SESSION['id']) && is_numeric($_SESSION['id'])){
        return true;
    }
    return false;
}

/* vérifie si un utilisateur est un administrateur connecté */
function is_admin(){
    if(is_connected() && !empty($_SESSION['is_admin']) && $_SESSION['is_admin'] == 1){
        return true;
    }
    return false;
}
